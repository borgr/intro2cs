...
def rpsls_game():
    ...
    plW,coW,dr=0,0,0 #player computer and draw counters
 ...
 (Paper), 3 (Scissors), 4 (Lizard) or 5 (Spock): "))
        if plCh>0 and plCh<6:
            ...
            coCh=random.randint(1,5) #computer choice
            strComp=get_selection(coCh)
            coCh=coCh-1
            print("    Computer has selected: %s." % strComp)
...
def rpsls_play():
    #This function plays the game as many sets as wanted.
    print("Welcome to the Rock-Scissors-Paper-Lizard-Spock game!")
    again=2 #A variable that checks if you want to continue/reset/quit.
    stCnt=0
    stW=0
    while again!=1:
        stCnt= stCnt+1
        ...
        if plW>coW:
            ...
            print("Too bad! You have lost in %i games." %gmCnt)
